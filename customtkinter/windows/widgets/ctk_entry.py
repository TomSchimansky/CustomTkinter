        ...
        def _on_root_click(self, event):
            try:
                if not self._is_click_inside(event):
                    if self._root_window.focus_get() == self._entry:
                        self._root_window.focus_set()
            except Exception as e:
                print(f"Error in _on_root_click: {e}")
        try:
            if not self._is_click_inside(event):
                # Check if the CTkEntry widget currently has the focus
                if self._root_window.focus_get() == self._entry:
                    # Set focus to the root window to ensure CTkEntry loses focus
                    self._root_window.focus_set()
        except Exception as e:
            print(f"Error in _on_root_click: {e}")
