...
class CTkScrollableFrame(tkinter.Frame, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    ...
    def _update_scrollbar(self, border_spacing=None):
        ...
        if self._orientation == "horizontal":
            if self._parent_canvas.xview() != (0.0, 1.0):
                self._parent_canvas.grid_configure(pady=(border_spacing, 0))
                self._scrollbar.grid(row=2, column=0, sticky="nsew", padx=border_spacing)
            else:
                self._parent_canvas.grid_configure(pady=border_spacing)
            ...
            if self._parent_canvas.yview() != (0.0, 1.0):
                ...
                self._parent_canvas.grid_configure(padx=border_spacing)
                self._scrollbar.grid_remove()
